...
# Поля ввода данных (используем текстовый ввод, чтобы поля были изначально пустыми)
x_input = st.text_input("Вставьте или введите первое число (X):", value="", placeholder="Например: 526.44")
y_input = st.text_input("Вставьте или введите второе число (Y):", value="", placeholder="Нажмите Enter для 75.66, если поле пустое")
...
# Валидация и перевод текста в числа
if x_input:
    try:
...
if y_input:
    ...
    # Если X введен, а Y пустой — подставляем значение по умолчанию 75.66
    y = 75.66
...
if x is not None and y is not None and not error_msg:
    ...
    # Кнопка со встроенным JavaScript для работы с буфером обмена (копирование)
    ...
    st.info("👋 Пожалуйста, введите число X, чтобы увидеть результат расчета.")
